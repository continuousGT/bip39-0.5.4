(function() {
    ...
    var DOM = {};
    //DOM.privacyScreenToggle = $(".privacy-screen-toggle");  // just for display Screen  Hide all privacy info
    //DOM.network = $(".network");
    ...
    //DOM.extendedPrivKey = $(".extended-priv-key");  // เป็นตัวแปรรับค่าจากการคำนวณ
        ...
        if(DOM_autoCompute) {
            ...
            phraseChangeTimeoutEvent = setTimeout(function() {
                ...
                if (entropy !== null) {
                    ...
                    DOM_entropy_val = entropy; //DOM.entropy.val(entropy);
                    //DOM.entropyTypeInputs.filter("[value='hexadecimal']").prop("checked", true);
                    DOM_entropyTypeInputs_val = "hexadecimal";
...
/*    function pbkdf2RoundsChanged() {
    ...
    /*function entropyChanged() {
        ...
        if (DOM_entropy_val.trim().length == 0) {
            clearDisplay();
            clearEntropyFeedback();
            DOM.phrase.val("");
            //DOM.phraseSplit.val("");
...
/*    function entropyTypeChanged() {
    ...
    /*function delayedSeedChanged() {
        ...
        if (DOM.phrase.val().length > 0 || DOM_passphrase_val.length > 0) {
        ...
        DOM.phrase.val("");
        //DOM.phraseSplit.val("");
        DOM_passphrase_val = "";//DOM.passphrase.val("");
        DOM.rootKey.val("");
        clearAddressesList();
        clearDerivedKeys();
        seed = null;
    ...
    function TableRow(index, isLast) {
        ...
        function calculateValues() {
            setTimeout(function() {
                ...
                /*if (networksName == "BCH - Bitcoin Cash") {
                ...
                // Bitcoin Cash address format may vary
                /*if (networksName == "SLP - Simple Ledger Protocol") {
                    var bchAddrType = DOM.bitcoinCashAddressType.filter(":checked").val();
                    if (bchAddrType == "cashaddr") {
                ...
                /*if (networksName == "ZBC - ZooBlockchain") {
...
/*    function isUsingOwnEntropy() {
    ...
    /*function setMnemonicFromEntropy() {
        clearEntropyFeedback();
        // Get entropy value
        var entropyStr = DOM_entropy_val; //DOM.entropy.val();
        ...
        if (entropyTypeAutoDetect) {
            ...
            let base = DOM_entropyTypeInputs_val; //DOM.entropyTypeInputs.filter(":checked").val();
        ...
        var mnemonicLength = DOM_entropyMnemoicLength; //DOM.entropyMnemonicLength.val();
    ...
    function clearEntropyFeedback() {
        ...
        DOM_entropyWordCount = "0"; // DOM.entropyWordCount.text("0");
        DOM_entropyEventCount = "0"; // DOM.entropyEventCount.text("0");
        DOM_entropyBitsPerEvent = "0";// DOM.entropyBitsPerEvent.text("0");
        DOM_entropyBits = "0"; // DOM.entropyBits.text("0");
    ...
    function showEntropyFeedback(entropy) {
        ...
        DOM_entropyEventCount = entropy.base.events.length;// DOM.entropyEventCount.text(entropy.base.events.length);
        DOM_entropyBits = numberOfBits; // DOM.entropyBits.text(numberOfBits);
        DOM_entropyWordCount = wordCount; //DOM.entropyWordCount.text(wordCount);
        //DOM.entropyBinary.text(spacedBinaryStr);
        DOM_entropyBitsPerEvent = bitsPerEvent; // DOM.entropyBitsPerEvent.text(bitsPerEvent);
        // detect and warn of filtering
        var rawNoSpaces = DOM_entropy_val.replace(/\s/g, ""); //DOM.entropy.val().replace(/\s/g, "");
